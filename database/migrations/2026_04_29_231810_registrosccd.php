...
return new class extends Migration
{
    ...
    {
        Schema::create('registrosccd', function (Blueprint $table) {
            $table->engine = "InnoDB";
            $table->bigIncrements('id');
            $table->bigInteger('id_cabeceraccd')->unsigned();
            $table->string('acto_administrativo', 3000);
            $table->string('funcion', 3000);
            $table->bigInteger('id_seccion')->unsigned();
            $table->bigInteger('id_subseccion')->unsigned()->nullable();
            $table->bigInteger('id_serie')->unsigned();
            $table->bigInteger('id_subserie')->unsigned()->nullable();
            $table->timestamps();

            $table->foreign('id_cabeceraccd')->references('id')->on('cabeceraccd')->onDelete('cascade');
