                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <span id="card_title" style="font-size: 1.5rem; font-weight: bold;">
                             ...
                             <div class="float-right">
                                @can('Create:Subseries')
                                <a href="{{ route('subseries.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                    Crear Subserie
                                </a>
                                @endcan
                              </div>
                                ...
                                <tbody>
                                    @foreach ($subseries as $subseriess)
                                        ...
                                        <td >{{ $subseriess->series->nombre }}</td>
                                            <td>
                                                <form action="{{ route('subseries.destroy', $subseriess->id) }}" method="POST">
                                                    @can('View:Subseries')
                                                    <a class="btn btn-sm btn-primary " href="{{ route('subseries.show', $subseriess->id) }}"><i class="fa fa-fw fa-eye"></i>Ver</a>
                                                    @endcan
                                                    @can('Update:Subseries')
                                                    <a class="btn btn-sm btn-success" href="{{ route('subseries.edit', $subseriess->id) }}"><i class="fa fa-fw fa-edit"></i>Editar</a>
                                                    @endcan
                                                    @can('Delete:Subseries')
                                                    @csrf
                                                    @method('DELETE')
