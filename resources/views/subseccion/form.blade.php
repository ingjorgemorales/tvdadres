<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <select name="id_seccion" class="form-control @error('id_seccion') is-invalid @enderror" id="id_seccion">
                ...
                @foreach($secciones as $id => $nombre)
                    <option value="{{ $id }}" {{ old('id_seccion', $subsecciones?->id_seccion) == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mb-2 mb20">
            ...
            <input type="text" name="codigo" class="form-control @error('codigo') is-invalid @enderror" value="{{ old('codigo', $subsecciones?->codigo) }}" id="codigo" placeholder="Codigo">
            {!! $errors->first('codigo', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="form-group mb-2 mb20">
            <label for="nombre" class="form-label">Nombre</label>
            <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre', $subsecciones?->nombre) }}" id="nombre" placeholder="Nombre">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <select name="id_tipo" class="form-control @error('id_tipo') is-invalid @enderror" id="id_tipo">
                ...
                @foreach($tiposeccion as $id => $nombre)
                    <option value="{{ $id }}" {{ old('id_tipo', $subsecciones?->id_tipo) == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
